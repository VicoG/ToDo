...
<!-- Document Management Dropdown -->
 ...
 if (isset($_POST['submit'])) {
	...
	switch ($doc) {
		case 'doc1':
			$query= "UPDATE main_table SET doc1= NULL WHERE id=$id LIMIT 1";
			break;
		case 'doc2':
			$query= "UPDATE main_table SET doc2= NULL WHERE id=$id LIMIT 1";
			break;
		case 'doc3':
			$query= "UPDATE main_table SET doc3= NULL WHERE id=$id LIMIT 1";
			break;
		case 'doc4':
			$query= "UPDATE main_table SET doc4= NULL WHERE id=$id LIMIT 1";
			break;
		case 'doc5':
			$query= "UPDATE main_table SET doc5= NULL WHERE id=$id LIMIT 1";
			break;
		case 'doc6':
			$query= "UPDATE main_tabl  SET doc6= NULL WHERE id=$id LIMIT 1";
			break;
		default:
			break;
	}
	
	$docDelete = mysqli_query($conn, $query);
	
	if(!$docDelete) { ?>
	<script type="text/javascript">
		alert("Delete Doc update failed.");
		</script>
<?php } else {
		unlink("docs/" . $name);	
	}
	...
	// Database query
	$queryDocs = "SELECT id, page, transit, doc1, doc2, doc3, doc4, doc5, doc6 FROM main_table";

	$completedResult = mysqli_query($conn, $queryDocs);
	//Test if there was a query error
	if(!$completedResult) { ?>
	<script type="text/javascript">
...
<section id="content" class="table-layout animated fadeIn"><!-- Begin: Content -->
				
<div class="panel-body p20" id="invoice-item">            
	<div class="row mt40 pt40 col-md-10 col-md-offset-1 pull-left">
		<div class="col-md-12 table-responsive">
		<div class="input-group"> <span class="input-group-addon">Search </span>
    		<input id="filter" type="text" class="form-control" placeholder="Type here...">
		</div> 
		<div id="printTable">
		<table class="table table-hover table-bordered text-muted" id="printTable">
		<thead>
			<tr class="primary">
				<th style="width: 40%">Document Name</th>
				<th>Phase Location</th>
				<th>Transit or Responder Specific</th>
				<th>Actions</th> 
			</tr>
		</thead>
		<?php	//Populate Table
		  ...
		  <tbody class="searchable">
			<?php if (!empty($Row['doc1'])){ ?>
				<tr>
				<td><a href="http://tid-appsvr.llnl.gov/utr/dev/docs/<?php	echo $Row["doc1"]; ?>" target="_blank" ><?php echo $Row["doc1"]; ?></a></td>
				<td><h4><?php	echo $Row["page"]; ?></h4></td>
				<td><h4><?php	 if( $Row["transit"] == 0){ echo "Responder";} else if( $Row["transit"] == 1){ echo "Transit";}; ?></h4></td>
				<td>
					<form method="post" action="documents.php" role="form">
						<input type="hidden" name="id" value="<?php	echo $Row["id"]; ?>" >
						<input type="hidden" name="docType" value="doc1">
						<input type="hidden" name="name" value="<?php	echo $Row['doc1']; ?>">
						<button type="submit" name="submit" onClick="return confirm('Are you sure you want to delete this item?');"  data-toggle="tooltip" title="Delete File" class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i></button>
					</form>
				</td>
				</tr>
			<?php } ?>
			<?php if (!empty($Row['doc2'])){ ?>
				<tr>
				<td> <a href="http://tid-appsvr.llnl.gov/utr/dev/docs/<?php	echo $Row["doc2"]; ?>" target="_blank" ><?php	echo $Row["doc2"]; ?></a></td>
				<td><h4><?php	echo $Row["page"]; ?></h4></td>
				<td><h4><?php	 if( $Row["transit"] == 0){ echo "Responder";} else if( $Row["transit"] == 1){ echo "Transit";}; ?></h4></td>
				<td>
					<form method="post" action="documents.php" role="form">
						<input type="hidden" name="id" value="<?php	echo $Row["id"]; ?>" >
						<input type="hidden" name="docType" value="doc1">
						<input type="hidden" name="name" value="<?php	echo $Row['doc2']; ?>">
						<button type="submit" name="submit" onClick="return confirm('Are you sure you want to delete this item?');"  data-toggle="tooltip" title="Delete File" class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i></button>
					</form>	
				</td>
				</tr>
			...
			<?php if (!empty($Row['doc3'])){ ?>
				<tr>
				<td> <a href="http://tid-appsvr.llnl.gov/utr/dev/docs/<?php	echo $Row["doc3"]; ?>" target="_blank" ><?php	echo $Row["doc3"]; ?></a></td>
				<td><h4><?php	echo $Row["page"]; ?></h4></td>
				<td><h4><?php	 if( $Row["transit"] == 0){ echo "Responder";} else if( $Row["transit"] == 1){ echo "Transit";}; ?></h4></td>
				<td>
					<form method="post" action="documents.php" role="form">
						<input type="hidden" name="id" value="<?php	echo $Row["id"]; ?>" >
						<input type="hidden" name="docType" value="doc1">
						<input type="hidden" name="name" value="<?php	echo $Row['doc3']; ?>">
						<button type="submit" name="submit" onClick="return confirm('Are you sure you want to delete this item?');"  data-toggle="tooltip" title="Delete File" class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i></button>
					</form>
				</td>
				</tr>
			...
			<?php if (!empty($Row['doc4'])){ ?>
				<tr>
				<td> <a href="http://tid-appsvr.llnl.gov/utr/dev/docs/<?php	echo $Row["doc4"]; ?>" target="_blank" ><?php	echo $Row["doc4"]; ?></a></td>
				<td><h4><?php	echo $Row["page"]; ?></h4></td>
				<td><h4><?php	 if( $Row["transit"] == 0){ echo "Responder";} else if( $Row["transit"] == 1){ echo "Transit";}; ?></h4></td>
				<td>
					<form method="post" action="documents.php" role="form">
						<input type="hidden" name="id" value="<?php	echo $Row["id"]; ?>" >
						<input type="hidden" name="docType" value="doc1">
						<input type="hidden" name="name" value="<?php	echo $Row['doc4']; ?>">
						<button type="submit" name="submit" onClick="return confirm('Are you sure you want to delete this item?');"  data-toggle="tooltip" title="Delete File" class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i></button>
					</form>
				</td>
				</tr>
			...
			<?php if (!empty($Row['doc5'])){ ?>
				<tr>
				<td> <a href="http://tid-appsvr.llnl.gov/utr/dev/docs/<?php	echo $Row["doc5"]; ?>" target="_blank" ><?php	echo $Row["doc5"]; ?></a></td>
				<td><h4><?php	echo $Row["page"]; ?></h4></td>
				<td><h4><?php	 if( $Row["transit"] == 0){ echo "Responder";} else if( $Row["transit"] == 1){ echo "Transit";}; ?></h4></td>
				<td>
					<form method="post" action="documents.php" role="form">
						<input type="hidden" name="id" value="<?php	echo $Row["id"]; ?>" >
						<input type="hidden" name="docType" value="doc1">
						<input type="hidden" name="name" value="<?php	echo $Row['doc5']; ?>">
						<button type="submit" name="submit" onClick="return confirm('Are you sure you want to delete this item?');"  data-toggle="tooltip" title="Delete File" class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i></button>
					</form>
				</td>
				</tr>
			<?php } ?>
			<?php if (!empty($Row['doc6'])){ ?>
				<tr>
				<td> <a href="http://tid-appsvr.llnl.gov/utr/dev/docs/<?php	echo $Row["doc6"]; ?>" target="_blank" ><?php	echo $Row["doc6"]; ?></a></td>
				<td>
					<form method="post" action="documents.php" role="form">
						<input type="hidden" name="id" value="<?php	echo $Row["id"]; ?>" >
						<input type="hidden" name="docType" value="doc1">
						<input type="hidden" name="name" value="<?php	echo $Row['doc6']; ?>">
						<button type="submit" name="submit" onClick="return confirm('Are you sure you want to delete this item?');"  data-toggle="tooltip" title="Delete File" class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i></button>
					</form>
				</td>
				</tr>
	...
	<br />
	<br />
	<div class="row col-md-10 col-md-offset-1 pull-left" id="invoice-footer">
...
<!-- BEGIN: PAGE SCRIPTS -->
  ...
  jQuery(document).ready(function() {
	  ...
	  (function ($) {     //Search table function
